...
import envoyerEmailIdentifiants from "../../email/approuverUser";
import ConfirmModalDelete from "../modal/ConfirmDelete";
...
const UserManagement: React.FC<UserManagementProps> = () => {
  ...
  const loadUsers = async (): Promise<void> => {
    try {
      const response = await api.get<ApiResponse>("/api/users");
      console.log("valeur request", response.data);
      setUsers(response.data.users);
      console.log("user récupéré", response.data.users);
    } catch (error) {
  ...
  const handleEmailPrefixChange = (value: string): void => {
    // Nettoyer la valeur : enlever espaces, caractères spéciaux sauf point et tiret
    const cleanValue = value
      .toLowerCase()
      .replace(/[^a-z0-9.-]/g, "")
      .replace(/@.*$/, ""); // Enlever tout ce qui suit un @

    setEmailPrefix(cleanValue);

    // Construire l'email complet
    const fullEmail = cleanValue ? `${cleanValue}${EMAIL_DOMAIN}` : "";
    setProfessionalData({
      ...professionalData,
      emailPro: fullEmail,
    });
  ...
  const approuver = async (): Promise<void> => {
    if (
      !selectedUser ||
    ...
    // Vérifier que l'email se termine bien par @serdi.cm
    if (!professionalData.emailPro.endsWith(EMAIL_DOMAIN)) {
      toast.error(`L'email doit se terminer par ${EMAIL_DOMAIN}`);
      return;
    }
    ...
    try {
      ...
      if (res.data.success) {
        ...
        // console.log("donnéer du json ", userForJson);
        // console.log("donnéer du json id user ", userForJson.id);
        // await axios.post(
  ...
  //gérer le modal du gérand
  const rejeter = async (userId: string | number): Promise<void> => {
    try {
      setUsers(
      ...
      console.log("Erreur rejet ", error);
      toast.error("Erreur lors du rejet");
    }
  };

  const handleOpenApprovalModal = (personne: User): void => {
    setSelectedUser(personne);

    // Suggestion automatique basée sur le nom et prénom
    const suggestedPrefix = `${personne.prenom}.${personne.nom}`
      .toLowerCase()
      .normalize("NFD")
    ...
    setProfessionalData({
      emailPro: `${suggestedPrefix}${EMAIL_DOMAIN}`,
              ...
              {users.map((personne) => (
                ...
                >
                  ...
                  <td className="py-4 px-6 space-x-2">
                    <button
                      onClick={() => handleOpenApprovalModal(personne)}
              ...
              <div>
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full px-4 py-3 bg-gray-50 dark:bg-white/10 border border-gray-300 dark:border-white/20 rounded-xl text-gray-800 dark:text-white placeholder-gray-400 dark:placeholder-white/50 focus:outline-none focus:ring-2 focus:ring-blue-500 pr-28"
                    placeholder="prenom.nom"
                  />
                  <div className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-500 dark:text-white/50 text-sm font-medium pointer-events-none">
                    @serdi.cm
                  </div>
                </div>
                {emailPrefix && (
