...
import DeleteTeamsModal from "./deleteTeams";
...
const Permissions: React.FC = () => {
  ...
  const [selectRoles, setSelectRoles] = useState<Role | null>(null);
  const [selectPermissions, setSelectPermissions] = useState<Permission[]>([]);
  const [selectTeams, setSelectTeams] = useState<Team[]>([]);
  const [showModal, setShowModal] = useState<boolean>(false);
  const [loading, setLoading] = useState<boolean>(false);
  const [showDeleteModal, setShowDeleteModal] = useState<boolean>(false);
  const [showDepartementModal, setShowDepartementModal] =
  ...
  const charger = async (): Promise<void> => {
    try {
      const res = await api.get<PermissionsApiResponse>("/api/permission");
  ...
  const RoleSelection = (role: Role): void => {
    setSelectRoles(role);
  };

  const PermissionSelection = (permission: Permission): void => {
  ...
  const FormChange = (e: ChangeEvent<HTMLInputElement>): void => {
  ...
  const Submit = async (e: FormEvent<HTMLFormElement>): Promise<void> => {
    ...
    try {
      ...
      if (res.data.success) {
        setTimeout(() => {
          toast.success(res.data.message);
          const updatedUser: User = { ...selectUser!, ...res.data.user! };
          setSelectUser(updatedUser);
          setUsers(
            users.map((u) => (u.id === selectUser!.id ? updatedUser : u)),
          );
  ...
  const handleTeamsDeleted = (deletedTeamIds: number[]): void => {
    setTeams((prevTeams) =>
          ...
          <form onSubmit={Submit} className="space-y-5">
            ...
            <div className="flex gap-3 mt-8">
              <button
                type="button"
                onClick={closeModal}
                className="flex-1 px-6 py-3 bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-200 font-medium rounded-xl transition-all"
              >
                Annuler
              </button>
              <button
  ...
  return (
    <div className="bg-gray-50 dark:bg-gray-900 min-h-screen">
      ...
      <div className="max-w-7xl mx-auto px-6 pb-8">
        <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
          <div className="flex flex-col lg:flex-row min-h-[600px]">
            ...
            <div className="lg:w-1/3 border-r border-gray-200 dark:border-gray-700 bg-gray-50 dark:bg-gray-900 flex flex-col">
                      ...
                      <div
                        ...
                        className={`p-4 rounded-xl cursor-pointer transition-all ${
                          selectUser?.id === user.id
                            ? "bg-purple-50 dark:bg-purple-900/30 border-2 border-purple-500 dark:border-purple-400 shadow-md"
                            : "bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 hover:border-purple-300 dark:hover:border-purple-600 hover:shadow-sm"
                        }`}
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {user.role && (
                              <div className="text-xs text-purple-600 dark:text-purple-400 mt-1 flex items-center font-medium">
                                {user.role.nom}
                              </div>
                            )}
                          </div>
                  ...
                  <div className="bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-2xl p-6">
                    <div className="flex items-center justify-between mb-6">
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="bg-purple-600 dark:bg-purple-500 hover:bg-purple-700 dark:hover:bg-purple-600 text-white px-4 py-2 rounded-xl transition-colors flex items-center shadow-sm"
                        >
                          <svg
                        ...
                        <button
                          ...
                          className="px-4 py-2 bg-red-600 dark:bg-red-500 text-white rounded-xl hover:bg-red-700 dark:hover:bg-red-600 transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-sm"
                        >
                          <svg
                            className="w-4 h-4"
                            fill="none"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                      <div className="bg-orange-50 dark:bg-orange-900/20 border border-orange-200 dark:border-orange-800 rounded-xl p-4">
                        <div className="font-semibold text-orange-700 dark:text-orange-400 mb-3 flex items-center text-sm">
                          <svg
                        ...
                        {selectUser.role ? (
                          <span className="inline-flex items-center px-3 py-1 rounded-full text-sm bg-orange-100 dark:bg-orange-900/40 text-orange-700 dark:text-orange-300 border border-orange-300 dark:border-orange-700 font-medium">
                            {selectUser.role.nom}
                          </span>
                        ) : (
                      ...
                      <div className="bg-green-50 dark:bg-green-900/20 border border-green-200 dark:border-green-800 rounded-xl p-4">
                        <div className="font-semibold text-green-700 dark:text-green-400 mb-3 flex items-center text-sm">
                          <svg
                            className="w-5 h-5 mr-2"
                            fill="none"
                            stroke="currentColor"
                          ...
                          <div className="flex flex-wrap gap-1">
                            {selectUser.permissions
                              ...
                              .map((permission) => (
                                <span
                                  key={permission.id}
                                  className="inline-flex items-center px-2 py-1 rounded-lg text-xs bg-green-100 dark:bg-green-900/40 text-green-700 dark:text-green-300 border border-green-300 dark:border-green-700 font-medium"
                                >
                      ...
                      <div className="bg-purple-50 dark:bg-purple-900/20 border border-purple-200 dark:border-purple-800 rounded-xl p-4">
                        <div className="font-semibold text-purple-700 dark:text-purple-400 mb-3 flex items-center text-sm">
                          <svg
                            className="w-5 h-5 mr-2"
                            fill="none"
                            stroke="currentColor"
                        ...
                        {selectUser.teams && selectUser.teams.length > 0 ? (
                          <div className="flex flex-wrap gap-1">
                            {selectUser.teams.slice(0, 3).map((team) => (
                              <span
                                key={team.id}
                                className="inline-flex items-center px-2 py-1 rounded-lg text-xs bg-purple-100 dark:bg-purple-900/40 text-purple-700 dark:text-purple-300 border border-purple-300 dark:border-purple-700 font-medium"
                              >
                                {team.nom}
                              </span>
                            ))}
                              ...
                              <button
                                ...
                                className={`p-3 rounded-lg text-sm font-medium transition-all border-2 ${
                                  selectRoles?.id === role.id
                                    ? "bg-orange-100 dark:bg-orange-900/40 border-orange-500 dark:border-orange-400 text-orange-700 dark:text-orange-300 shadow-md"
                                    : "bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-orange-300 dark:hover:border-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20"
                                }`}
                              >
                                {role.nom}
                              </button>
                                ...
                                <button
                                  ...
                                  className={`p-3 rounded-lg text-sm font-medium transition-all border-2 ${
                                    selectPermissions.some(
                                      ...
                                      ? "bg-green-100 dark:bg-green-900/40 border-green-500 dark:border-green-400 text-green-700 dark:text-green-300 shadow-md"
                                      : "bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-green-300 dark:hover:border-green-600 hover:bg-green-50 dark:hover:bg-green-900/20"
                                  }`}
                                >
                                  {permission.description}
                                </button>
                              ...
                              <button
                                ...
                                className={`p-3 rounded-lg text-sm font-medium transition-all border-2 ${
                                  selectTeams.some((t) => t.id === team.id)
                                    ? "bg-purple-100 dark:bg-purple-900/40 border-purple-500 dark:border-purple-400 text-purple-700 dark:text-purple-300 shadow-md"
                                    : "bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:border-purple-300 dark:hover:border-purple-600 hover:bg-purple-50 dark:hover:bg-purple-900/20"
                                }`}
                              >
                                {team.nom}
                              </button>
      ...
      <button
        ...
        className="fixed bottom-8 right-8 bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 dark:from-indigo-500 dark:to-indigo-600 dark:hover:from-indigo-600 dark:hover:to-indigo-700 text-white p-4 rounded-full shadow-2xl hover:shadow-3xl transition-all duration-300 hover:scale-110 z-40"
